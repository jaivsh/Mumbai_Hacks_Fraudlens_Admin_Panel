...
**Federated-style GCP round (two silo CSVs → one global model):** see [`federated/README.md`](federated/README.md) and `federated_aggregate_train.py` (Cloud Run Job). Your `synthetic_transactions_source2.csv` is intended for the **bank-b** GCS prefix; pair it with a **bank-a** export or use `federated/split_for_demo_nodes.py` to simulate two nodes from one file.
